<script lang="ts">
    ...
    let { category, asset, thumbUrl, assetUrl, zoomIn = $bindable(), zoomOut = $bindable(), zoomReset = $bindable(), onwheel = $bindable(), rotateRight = $bindable() }: Props = $props();
    ...
    function onWheel(e: WheelEvent) {
        ...
        wheelTimer = setTimeout(() => { animated = true }, 150)
        const rect = container.getBoundingClientRect()
        applyZoom(scale + (e.deltaY > 0 ? -STEP : STEP), e.clientX - rect.left, e.clientY - rect.top)
    }
    ...
    $effect(() => {
        ...
        zoomReset = () => { scale = 1; translateX = 0; translateY = 0 }
        onwheel = onWheel
...
<div
    bind:this={container}
    class="asset-preview-full min-w-xl max-w-3xl max-h-[80vh]"
    style="transform: scale({scale}) rotate({rotation}deg); transform-origin: center; transition: {animated ? 'transform 0.25s cubic-bezier(0.25, 0.46, 0.45, 0.94)' : 'none'}; cursor: {scale > 1 ? 'grab' : 'default'};"
>
    {#if category === 'image'}
        <img
            src={assetUrl}
