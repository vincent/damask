<script lang="ts">
  import { invalidateAIProviderModelsCache } from '$lib/api/assets'
  ...
  async function refreshStatus() {
    try {
      status = await aiProvidersApi.getAIProviderKeyStatus('imagerouter')
    } catch {}
  }
  ...
  async function save() {
    ...
    testResult = null
    try {
      await aiProvidersApi.setAIProviderKey('imagerouter', keyInput.trim())
      keyInput = ''
      showForm = false
      invalidateAIProviderModelsCache()
      await refreshStatus()
    } finally {
      saving = false
    }
  ...
  async function clear() {
    ...
    testResult = null
    try {
      await aiProvidersApi.clearAIProviderKey('imagerouter')
      confirmClear = false
      invalidateAIProviderModelsCache()
      await refreshStatus()
    } finally {
  ...
  async function test() {
    ...
    testResult = null
    try {
      await aiProvidersApi.testAIProviderKey('imagerouter')
      testResult = 'success'
    } catch (e) {
      testResult =
