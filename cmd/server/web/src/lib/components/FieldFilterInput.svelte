...
<div class="flex flex-col gap-1 min-w-0">
  <label for="field-{def.key}" class="text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500">
    {def.name}
  </label>
  ...
  {:else if def.field_type === 'number'}
    {@const nums = { ...{ min: '', max: '' }, ...local[def.key] as { min: string; max: string } }}
    <div class="flex items-center gap-1">
      <input
        ...
        placeholder="min"
        value={nums.min}
        oninput={(e) => { (local[def.key] as { min: string; max: string }).min = (e.target as HTMLInputElement).value; ondebouncedchange() }}
      ...
      <input
        ...
        placeholder="max"
        value={nums.max}
        oninput={(e) => { (local[def.key] as { min: string; max: string }).max = (e.target as HTMLInputElement).value; ondebouncedchange() }}
        class="w-20 rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-sm text-gray-900
          focus:border-indigo-400 focus:outline-none
          dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
      />
  ...
  {:else if def.field_type === 'date'}
    ...
    <div class="flex items-center gap-1">
      <input
        type="date"
        value={dates?.from || ''}
        onchange={(e) => { (local[def.key] as { from: string; to: string }).from = (e.target as HTMLInputElement).value; onchange() }}
      ...
      <input
        type="date"
        value={dates?.to || ''}
        onchange={(e) => { (local[def.key] as { from: string; to: string }).to = (e.target as HTMLInputElement).value; onchange() }}
        class="rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-sm text-gray-900
          focus:border-indigo-400 focus:outline-none
          dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
      />
    ...
    <div class="flex gap-2">
      {#each [['', 'Any'], ['true', 'Yes'], ['false', 'No']] as [val, label]}
        <button
          ...
          onclick={() => { local[def.key] = val; onchange() }}
        >
          {label}
        </button>
      {/each}
        ...
        <button
          ...
          class="rounded-full border px-2.5 py-1 text-sm transition-colors
            {sv === opt
              ? 'border-indigo-500 bg-indigo-50 text-indigo-700 dark:border-indigo-400 dark:bg-indigo-950/40 dark:text-indigo-300'
              : 'border-gray-200 text-gray-600 hover:border-gray-300 dark:border-gray-700 dark:text-gray-400'}"
          onclick={() => { local[def.key] = sv === opt ? '' : opt; onchange() }}
        >
          {opt}
        </button>
      {/each}
