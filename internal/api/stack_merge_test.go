...
func TestGetJob_NotFound(t *testing.T) {
	...
	resp, err := env.App.Test(th.AuthRequest(http.MethodGet, "/api/v1/jobs/nonexistent", nil, owner.Cookie))
	if err != nil {
		t.Fatal(err)
	}
	defer resp.Body.Close()
...
func TestGetJob_Status(t *testing.T) {
	env, owner := th.SetupWithOwner(t)
	a1 := th.UploadAsset(t, env, owner.Cookie)
	a2 := th.UploadAsset(t, env, owner.Cookie)

	// Enqueue a merge job.
	mergeResp, err := env.App.Test(th.AuthRequest(http.MethodPost, "/api/v1/stack/merge",
		th.JsonBody(map[string]any{
			"asset_ids":   []string{a1.ID, a2.ID},
			"output_type": "pdf",
		}), owner.Cookie), fiber.TestConfig{Timeout: 5000})
	...
	var result map[string]any
	if err := json.NewDecoder(mergeResp.Body).Decode(&result); err != nil {
		t.Fatal(err)
	}
	...
	// Poll job status.
	resp, err := env.App.Test(th.AuthRequest(http.MethodGet, "/api/v1/jobs/"+jobID, nil, owner.Cookie),
