...
func (s *EmailAPISource) Validate(ctx context.Context) error {
	if s.cfg.IngestToken == "" {
		return fmt.Errorf("email_api: ingest_token is empty")
	}
	return nil
}
...
func (s *EmailAPISource) Poll(ctx context.Context) ([]ingress.IngestItem, error) {
	return nil, nil
}
...
func (s *EmailAPISource) Fetch(ctx context.Context, item ingress.IngestItem) (io.ReadCloser, error) {
	...
	return nil, fmt.Errorf("email_api: fetch not supported for push source")
}
