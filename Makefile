...
coverage:
	...
	@awk 'FNR==1{next} {print}' coverage/unit.out coverage/integration.out \
	  ...
	  >> coverage/merged.out
	@grep -vE '^(damask/server/internal/docs|damask/server/internal/testutil|damask/server/internal/testhelpers|damask/server/internal/repository/memory|damask/server/cmd/server/web/node_modules)/' \
