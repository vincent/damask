...
<div class="space-y-5">
    <div class="grid grid-cols-2 gap-4">
        <div>
            <label for="variant-{kind}-width" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">Opacity ({watermarkOpacity}%)</label>
            <input id="variant-{kind}-width" type="range" min="1" max="100" bind:value={watermarkOpacity} class="w-full accent-indigo-500" />
        </div>
        <div>
            <label for="variant-{kind}-height" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">Quality: {watermarkQuality}%</label>
